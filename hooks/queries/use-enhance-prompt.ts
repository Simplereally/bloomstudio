...
export function useEnhancePrompt(
  ...
  // Track if the current enhancement was cancelled to prevent onSuccess from firing
  ...
  >({
    ...
    onSuccess: (enhancedText, request) => {
      ...
      // Don't call onSuccess if the request was cancelled
    ...
    onError: (error, request) => {
      abortControllerRef.current = null
      // Don't call onError if the request was cancelled (abort errors are expected)
      if (cancelledRef.current) {
        cancelledRef.current = false
        return
      }
  ...
  const cancel = useCallback(() => {
    if (abortControllerRef.current) {
      // Mark as cancelled before aborting to handle race conditions
