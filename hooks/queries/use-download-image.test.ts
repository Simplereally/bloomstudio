...
import { PollinationsApiError, ClientErrorCodeConst } from "@/lib/api";
...
describe("useDownloadImage", () => {
  ...
  it("downloads an image successfully", async () => {
    ...
    const { result } = renderHook(() => useDownloadImage({ onSuccess }), { wrapper: createWrapper() });
  ...
  it("handles download error with PollinationsApiError type", async () => {
    const error = new PollinationsApiError("Download failed", ClientErrorCodeConst.GENERATION_FAILED, 500);
    mockDownloadImage.mockRejectedValueOnce(error);

    const onError = vi.fn();
    const { result } = renderHook(() => useDownloadImage({ onError }), { wrapper: createWrapper() });
    ...
    expect(result.current.error?.code).toBe(ClientErrorCodeConst.GENERATION_FAILED);
