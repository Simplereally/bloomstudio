    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <Button
          ...
          onClick={handleClick}
          disabled={isDisabled}
          className={`group absolute bottom-2 right-2 z-10 h-7 w-7 rounded-md bg-background/80 backdrop-blur-sm border border-border/50 shadow-sm hover:bg-muted hover:text-foreground transition-all duration-200 ${isEnhancing ? "text-destructive hover:text-destructive hover:bg-destructive/10" : ""} ${className || ""}`}
