...
import { ClientErrorCodeConst, ERROR_MESSAGES, PollinationsApiError, isFluxModelUnavailable } from "@/lib/errors"
...
export async function POST(request: NextRequest): Promise<NextResponse<ServerGenerateSuccess | ServerGenerateError>> {
    try {
        ...
        console.log("[/api/generate] Pollinations response headers:", JSON.stringify(headersObj, null, 2))

        if (!response.ok) {
            // Clone response so we can read the body twice (once for raw logging, once for typed parsing)
            const responseClone = response.clone()

            // Raw error with full depth for nested objects like fieldErrors
            const rawError = await responseClone.json()
            console.error("[/api/generate] Pollinations API error (raw):", JSON.stringify(rawError, null, 2))
            ...
            // Check for specific upstream errors and return user-friendly messages
            if (isFluxModelUnavailable(apiError.message)) {
                return NextResponse.json(
                    {
                        success: false,
                        error: {
                            code: ClientErrorCodeConst.MODEL_UNAVAILABLE,
                            message: ERROR_MESSAGES[ClientErrorCodeConst.MODEL_UNAVAILABLE],
                        },
                    },
                    { status: 503 }
                )
