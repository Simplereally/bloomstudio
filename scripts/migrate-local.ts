  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ffmpeg(inputPath)
        ...
        .outputOptions([
          "-vf",
          `crop=min(iw\\,ih):min(iw\\,ih),scale=128:128`,
          "-q:v",
          "6", // ~80% quality (0-31 scale, lower value is better quality)
        ])
        .output(outputPath)
        .on("end", () => resolve())
...
async function main() {
  ...
  while (!isDone) {
    ...
    interface ImagesBatchResult {
      page: Array<{ _id: Id<"generatedImages">; url: string; contentType: string; r2Key: string }>;
      isDone: boolean;
      continueCursor: string;
    }

    const result: ImagesBatchResult = await client.query(api.thumbnailMigration.getAllImagesBatch, {
      paginationOpts: {
        numItems: 20,
        cursor: cursor,
      },
