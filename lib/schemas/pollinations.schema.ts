...
export const ImageGenerationParamsSchema = z.object({
  ...
  model: ImageModelSchema.optional().default("zimage"),
  width: z.number().int().min(64).max(16384).optional().default(768),
  height: z.number().int().min(64).max(16384).optional().default(768),
  seed: z.number().int().min(0).max(1844674407370955).optional(),
  enhance: z.boolean().optional().default(false),
  quality: QualitySchema.optional().default("high"),
...
export const ApiErrorCodeSchema = z.enum(["BAD_REQUEST", "UNAUTHORIZED", "INTERNAL_ERROR"]);
...
export const InternalErrorSchema = z.looseObject({
  ...
  error: BaseApiErrorSchema.extend({
    ...
    message: z.union([z.literal("Oh snap, something went wrong on our end. We're on it!"), z.string()]),
    details: z
      .looseObject({
        name: z.string().optional(),
        stack: z.string().optional(),
...
export const ApiErrorSchema = z.union([BadRequestErrorSchema, UnauthorizedErrorSchema, InternalErrorSchema]);
