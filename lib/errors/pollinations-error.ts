...
export const ERROR_MESSAGES: Record<ErrorCode, string> = {
    ...
    [ClientErrorCodeConst.MODEL_UNAVAILABLE]: "The requested model is temporarily unavailable. Please try again later or use a different model.",
    [ClientErrorCodeConst.TRIAL_EXPIRED]: "Your 24-hour trial has expired",
} as const

/**
 * Known upstream error message patterns and their corresponding error codes
 * @deprecated Flux model has been decommissioned. This pattern is kept for backwards compatibility.
 */
const UPSTREAM_ERROR_PATTERNS = {
    /** @deprecated Flux has been decommissioned */
    FLUX_UNAVAILABLE: "No active flux servers available",
} as const

/**
 * Checks if an error message indicates the Flux model is unavailable.
 * Handles both plain messages and nested JSON error strings from Pollinations.
 * @deprecated Flux model has been decommissioned. This function is kept for legacy error handling only.
 */
export function isFluxModelUnavailable(errorMessage: string): boolean {
    // Direct match
    if (errorMessage.includes(UPSTREAM_ERROR_PATTERNS.FLUX_UNAVAILABLE)) {
        return true
    }

    // The Pollinations API sometimes returns errors with nested JSON in the message field
    // e.g., { "message": "{\"error\":\"Internal Server Error\",\"message\":\"No active flux servers available\",...}" }
    try {
        const parsed = JSON.parse(errorMessage)
        if (typeof parsed === "object" && parsed !== null) {
            const nestedMessage = parsed.message ?? parsed.error
            if (typeof nestedMessage === "string" && nestedMessage.includes(UPSTREAM_ERROR_PATTERNS.FLUX_UNAVAILABLE)) {
                return true
            }
        }
    } catch {
        // Not JSON, that's fine - we already checked the direct string
    }
...
export class PollinationsApiError extends Error {
    ...
    static fromInternalError(response: InternalError): PollinationsApiError {
        const { status, error } = response
        const extraDetails: PollinationsErrorDetails = { ...(error.details || {}) }
...
function getServerGenerationErrorMessage(code: string): string {
    ...
    const codeToMessage: Record<string, string> = {
        ...
        MODEL_UNAVAILABLE: ERROR_MESSAGES[ClientErrorCodeConst.MODEL_UNAVAILABLE],
        TRIAL_EXPIRED: ERROR_MESSAGES[ClientErrorCodeConst.TRIAL_EXPIRED],
    }
 ...
 * Checks the error code (from ServerGenerationError or PollinationsApiError).
 */
export function isTrialExpiredError(error: unknown): boolean {
    if (isServerGenerationError(error)) {
        return error.code === ClientErrorCodeConst.TRIAL_EXPIRED
    }
