  ...
  handler: async (ctx) => {
    ...
    if (existingUser) {
      ...
      if (
        ...
        identity.pictureUrl !== existingUser.pictureUrl
      ) {
 ...
 * The key is encrypted server-side before storage using AES-256-GCM.
 */
...
/**
 * Get the decrypted Pollinations API key for the current user.
 * Returns null if no key is set or user is not found.
 */
